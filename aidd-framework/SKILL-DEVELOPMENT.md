...
| 項目 | ルール |
|------|--------|
| frontmatter | `name`・`description` 必須。`name` はディレクトリ名と一致。**description はトリガー判定専用**（後述）|
...
### コンテキストウィンドウ管理（Inclusion Mode）（#1526）

スキルのコンテンツは読み込みタイミングによって 3 段階に分類される。スキル開発時にコンテンツをどのレベルに置くかを意識的に選択することで、コンテキストウィンドウの膨張を抑制する。

| レベル | 読み込みタイミング | 配置場所 | 内容 |
|-------|----------------|---------|------|
| **always**（常時）| スキルが呼び出された瞬間 | `SKILL.md`（本文）| Mission・インターフェース・重要ルール・承認ゲート |
| **manual**（明示参照）| Step 内で `references/` を参照した場合のみ | `references/*.md` | Step 詳細・テンプレート・レビュー指示 |
| **fork**（分離）| `context: fork` を指定した自己完結タスクのみ | frontmatter `context: fork` | 完全に独立した単発タスク（メイン会話履歴が不要）|

DO: コンテンツ配置の判断基準
- SKILL.md に置く: スキル実行中に常に参照が必要な情報
- references/ に置く: 特定 Step でのみ参照する重い詳細（500 行超えを防ぐ）
- context: fork を使う: 外部ツール呼び出し・一発ビルドなど会話履歴が不要なタスク

NEVER: スキルに無関係な情報を SKILL.md に含める（always-level に置くほど全スキル実行でコストを払う）。

### description フィールドの設計原則

MANDATORY: `description` はスキルのトリガー判定にのみ使用する。

NEVER: ワークフロー要約・手順・制約・ステップ概要を `description` に書く。
エージェントが `description` だけ読んでスキル本文をスキップする現象（description-skip）が発生するため。

```yaml
# Good: いつ呼ぶかのみ記述
description: >
  ユーザーが X をしたい場合・Y と Z の場面で使用する。

# Bad: 手順・制約を書いている
description: >
  X を実行し、Y を確認してから Z を行うスキル。
  Step 1: ... Step 2: ...
```
