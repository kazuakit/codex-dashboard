...
- コード探索（依存関係・影響範囲・既存コード調査）は codegraph を使用する。使用前に `codegraph_status` で状態確認し、`nodes == 0`（Stale）なら `npx @colbymchenry/codegraph index` を実行し、Uninitialized なら `/aidd-setup-codegraph` で初期化してから使用する。`grep` / `Read` / `Glob` へのフォールバック禁止
